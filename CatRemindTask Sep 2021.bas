...
' sets a specific due date
        ...
        .FlagRequest = UniConst(Umbrella) & " " & UniConst(XMark) & " " & objMsg.SenderName
        .ReminderSet = True
        .ReminderTime = Now + 4
' Now show Categories
...
'Create a corresponding task for reminders & follow up
        ...
        With objTask
            .Subject = UniConst(Umbrella) & " " & UniConst(XMark) & " " & " " & UniConst(PeaceLove) & " " & objMsg.Subject
...
Public Function UniConst(Text As eUnicodeConst) As String
    ...
    Select Case Text
        ...
        Case Umbrella:      UniConst = ChrW(9730)
...
Set objMsg = Application.CreateItem(olMailItem)
 ...
 With objMsg
  ...
  .Subject = UniConst(Umbrella) & " " & UniConst(XMark) & " " & UniConst(CheckOK) & " " & "El texto"
  .Categories = "Office"
